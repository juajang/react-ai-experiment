...
const DraggableText: React.FC<DraggableTextProps> = ({
  ...
  // useRef로 드래그 상태 관리 (이벤트 리스너가 항상 최신 값을 참조하도록)
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    if (isDraggingRef.current) {
      ...
      // 스냅 상태 콜백 (드래그 중임을 알림)
      if (onSnapChange) {
        onSnapChange({ horizontal: snapH, vertical: snapV, isDragging: true });
      }
    ...
    } else if (isResizingWidthRef.current) {
      const deltaX = e.clientX - dragStartPos.current.x;
  ...
  }, [onChange, onSnapChange, minFontSize, maxFontSize, snapThreshold]);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    // 드래그 종료 시 스냅 해제
    if (wasDragging && onSnapChange) {
      onSnapChange({ horizontal: false, vertical: false, isDragging: false });
    }
  }, [onSnapChange]);
